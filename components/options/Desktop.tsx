...
import desktopCompare from '@/data/desktopCompare.json';
	...
	return (
		<div className='container mx-auto h-fit w-full'>
			<div className='w-full h-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 lg:grid-cols-4 justify-center items-center'>
				{desktopCompare.map((mac) => (
					<div key={mac.id} className='border-b pb-2 w-full h-full'>
						<header className='relative w-60 h-60 aspect-square mx-auto'>
						...
						<div className='flex flex-col justify-center items-center gap-y-5 space-y-5'>
						...
						<div>
							{mac.data.map((data, i) => (
								<div
									key={i}
									className='flex flex-col justify-center w-full h-full space-y-5 gap-5 items-center'
								>
